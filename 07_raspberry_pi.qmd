...
-   Edit the "known_hosts" file by opening the known_hosts file. Use any text editor such as Notepad or Nano to open it.

```bash
C:\Users\phil/.ssh/known_hosts
```
-   Remove the line containing the IP of the Raspberry Pi:

-   Save the file and exit. (If using Nano: Hold **CTRL** and press **x**. Type press **y** to save, then press **Enter** to exit)
...
| **tmux** | `x.x.x` | Used to SSH into the Pi, via the internet |
...
```bash
sudo apt-get install git ntpdate tmux python3 minicom pip vnstat
```
...
| **pymcuprog** | `3.19.4.61` | Microchip PIC microcontroller programmer utility |

-   If you're having problems installing pip, try using:

```bash
sudo apt install python3-pip --fix-missing
```

-   To install the python packages, use pip: ***\$ pip install pyserial vcgencmd psutil pymcuprog*** and answer Yes to any questions that may appear:
